...
└───────────────────────────┬────────────────────────────┘
                            │
                            │ 2. 攔截請求，觸發 Depends(get_db)
                            ▼
┌────────────────────────────────────────────────────────┐
│                  資料庫連線生命週期 (get_db)            │
│  • 建立 Session (db = SessionLocal())                  │
│  • 透過 yield db 將連線借給 API 函式                     │
└───────────────────────────┬────────────────────────────┘
                            │
                            │ 3. 呼叫路由函式 get_all_employees(db)
                            ▼
┌────────────────────────────────────────────────────────┐
│                   SQLAlchemy ORM 查詢                  │
│  • 執行 db.query(Employee).all()                       │
│  • 將 Python 語法轉譯為 SQL: SELECT * FROM employees;  │
└───────────────────────────┬────────────────────────────┘
                            │
                            │ 4. 送出 SQL 語法向資料庫撈取資料
                            ▼
...
└───────────────────────────┬────────────────────────────┘
                            │
                            │ 5. 回傳原始資料列 (Database Rows)
                            ▼
┌────────────────────────────────────────────────────────┐
│               Pydantic 序列化 (EmployeeSchema)          │
│  • SQLAlchemy 將 Rows 封裝成 Employee ORM 物件          │
│  • Pydantic (from_attributes=True) 讀取物件             │
│  • 檢查型別、過濾欄位，並轉為 JSON 陣列                  │
└───────────────────────────┬────────────────────────────┘
                            │
                            │ 6. 觸發 get_db() 中的 finally 區塊
                            ▼
┌────────────────────────────────────────────────────────┐
│                    關閉連線 (db.close())                │
│  • 歸還/釋放 PostgreSQL 連線資源                        │
└───────────────────────────┬────────────────────────────┘
                            │
                            │ 7. 回傳 HTTP 200 OK 包含 JSON 資料
                            ▼
               [ 前端收到完整 JSON 資料 ]
```
...
[ 前端 / Streamlit ] 
        │
        │ 1. 發送 POST 請求 (帶有 JSON 資料)
        ▼
[ FastAPI 路由層 (main.py) ] ── (2. 觸發 Depends) ──► [ 資料庫連線 (database.py) ]
        │                                                     │
        │ 3. Pydantic 驗證 (schemas.py)                       │ 建立 db 連線
        ▼                                                     ▼
[ 執行 API 業務邏輯 (main.py) ] ◄─────────────────────────────┘
        │
        │ 4. ORM 轉換與操作 (models.py)
        ▼
[ PostgreSQL 資料庫 ]
        │
        │ 5. 回傳寫入結果
        ▼
[ Pydantic 序列化 (schemas.py) ]
        │
        │ 6. 自動轉為 JSON 並觸發 yield 歸還 db 連線
        ▼
[ 回傳 201 Created JSON 給前端 ]
```